                    ...
                    playerTwoCircleView?.constructors?.forEach { constructor ->
                        constructor.createAfterHook {
                            ...
                            (it.thisObject as ImageView).apply {
                                getNotificationElementBlendColors(context, getState == 1, isDarkMode(context))?.let { iArr -> setMiBackgroundBlendColors(iArr, 1f) }
                            }
                            ...
                            statusBarStateControllerImpl?.methodFinder()?.filterByName("getState")?.first()?.createAfterHook { hookParam1 ->
                                ...
                                if (lockScreenStatus == null || darkModeStatus == null || lockScreenStatus != isInLockScreen || darkModeStatus != isDarkMode) {
                                    ...
                                    (it.thisObject as ImageView).apply {
                                        getNotificationElementBlendColors(context, isInLockScreen, darkModeStatus)?.let { iArr -> setMiBackgroundBlendColors(iArr, 1f) }
                                    }
                                    if (mediaControlPanelInstance != null) {
                                        ...
                                        if (isBackgroundBlurOpened) {
                                            val mMediaViewHolder = mediaControlPanelInstance.objectHelper()
                                                .getObjectOrNullUntilSuperclass("mMediaViewHolder")
                                            if (mMediaViewHolder != null) {
                                                val action0 = mMediaViewHolder.objectHelper()
                                                    .getObjectOrNullAs<ImageButton>("action0")
                                                val action1 = mMediaViewHolder.objectHelper()
                                                    .getObjectOrNullAs<ImageButton>("action1")
                                                val action2 = mMediaViewHolder.objectHelper()
                                                    .getObjectOrNullAs<ImageButton>("action2")
                                                val action3 = mMediaViewHolder.objectHelper()
                                                    .getObjectOrNullAs<ImageButton>("action3")
                                                val action4 = mMediaViewHolder.objectHelper()
                                                    .getObjectOrNullAs<ImageButton>("action4")
                                                val titleText = mMediaViewHolder.objectHelper()
                                                ...
                                                val seekBar = mMediaViewHolder.objectHelper()
                                                ...
                                                val grey =
                                                    if (isDarkMode(context)) Color.LTGRAY else Color.DKGRAY
                                                val color =
